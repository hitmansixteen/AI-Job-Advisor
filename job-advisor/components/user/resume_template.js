...
const ResumeTemplate = ({ user }) => {
  ...
  return (
    <div style={{ minHeight: "1200px", maxWidth: "800px", marginLeft: "auto", padding: "20px", fontFamily: "'Times New Roman', Times, serif", lineHeight: "1.6", backgroundColor: "#f9f9f9", border: "1px solid #ddd", boxShadow: "0 0 10px rgba(0, 0, 0, 0.1)" }}>
      {/* Header */}
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <ul style={{ listStyleType: "disc", paddingLeft: "20px" }}>
            {user.skills.map((skill, index) => (
              <li key={index} style={{ fontSize: "16px", color: "#666", marginBottom: "5px" }}>{skill}</li>
            ))}
          </ul>
        </div>
      )}
